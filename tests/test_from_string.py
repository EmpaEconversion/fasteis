...
def test_from_string_series_inside_parallel_branches() -> None:
    ...
    assert circuit.param_names() == ["R0.r", "R1.r", "C1.c", "R2.r", "Cpe2.q", "Cpe2.alpha"]
...
def test_with_values_sets_params_positionally() -> None:
    circuit = eis.Circuit.from_string("R0-p(R1,Cpe1)").with_values([100.0, 200.0, 3e-4, 0.8])
    assert circuit.param_names() == ["R0.r", "R1.r", "Cpe1.q", "Cpe1.alpha"]
...
def test_from_string_circuit_can_be_fit() -> None:
    ...
    guess = eis.Circuit.from_string("R0-p(R1,W1)-C1").with_values([25.0, 150.0, 65.0, 1.3e-5])
