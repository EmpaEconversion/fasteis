...
CIRCUITS: dict[str, TrainingCircuit] = {
    ...
    for circuit in (
        ...
        # surface film plus a diffusing charge-transfer arc: the usual Li-ion model,
        # `R(RQ)(Q(RW))` in Boukamp notation
        # R2 and W2 sit in series in the same branch, so a Warburg strong enough to
        # see inside the window also hides R2; keeping its onset below the window
        # measured best (31% vs 43% of spectra with an unconstrained parameter)
